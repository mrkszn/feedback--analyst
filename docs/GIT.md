...
## 4. Ритм коммитов — обязательное правило

**Один завершённый этап = один коммит.** Этап в нашем случае — это либо:
- **Имплементация одной функции** из `docs/functions/` (после reviewer approve, до handoff'а следующей команде), или
- **Изменение инфраструктуры** (миграция БД, обновление окружения, правки `.env.example`/configs).

Запрещено:
- Откладывать коммит «до конца группы» или «до конца дня» — теряем гранулярность истории, ломаем bisect, ревью diff'а пухнет.
- Делать «комбо-коммит» на 2+ функции — даже если они смежные.
- Двигаться к следующей функции до того, как предыдущая закоммичена.

**Кто делает коммит:** team-lead команды функции, **после** того как reviewer дал ✅ approve и до `SendMessage` следующей команде. Это часть `/goal` функции.

**Формат коммита одной функции:**
```
feat(<group>): implement <function_name>

<1–3 строки WHY/что разблокируется>.
Closes docs/functions/<file>.md.
```
Где `<group>` — один из `utils`, `integrations`, `services`, `tools`, `agent`, `bot-guest`, `bot-admin`, `api`.

Пример:
```
feat(utils): implement download_voice_to_tmp

Скачивает voice-файл из Telegram во временный каталог. Базовая утилита
для voice-пайплайна; разблокирует integrations_whisper_transcribe.
Closes docs/functions/utils_voice_download.md.
```

**Push:** не обязателен после каждого коммита — допустимо копить локально и пушить пачкой раз в группу функций (`git push origin main`). Push в `permissions.ask`, требует подтверждения.
...
## 7. PR-чеклист (агенты обязаны выполнять)
...
## 8. Запрещённые операции без явного OK человека
