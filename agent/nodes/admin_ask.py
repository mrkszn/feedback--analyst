...
ADMIN_ASK_SYSTEM = (
    ...
    "(только для number-вопросов: avg/min/max);\n"
    "- categorical_distribution_tool(metric_key, days) — распределение по "
    "категориям для enum/boolean (возрастные группы, способ заказа, частота, "
    "yes/no и пр.);\n"
    ...
    "3. Для «сколько каких / процент / разбивка по» enum-вопросам — "
    "categorical_distribution_tool (НЕ aggregate_metric_tool: он только "
    "для number).\n"
    "4. Если уместен ASCII-график (тренд по дням 3-7 точек или распределение) "
    "— нарисуй его плоским текстом ниже основного ответа.\n"
    "5. Если запрос неоднозначный — кратко переспроси, не вызывай инструменты "
    "наугад."
)
